...
Exception declarations at governance level must be recorded through the Janus Core HUMAN_DECISION mechanism as defined in RFC 0004.
All declared exceptions MUST be registered in a centralized exception log maintained alongside the protocol stack. Unregistered exceptions are non-compliant regardless of justification.
This protocol applies across all layers. Its canonical location is the Framework Stack. This instance applies specifically to the Frontend Product Layer pending framework-level formalization.
