...
Cross-layer conflicts require a framework-level hierarchy declaration before this protocol can resolve them. In absence of such declaration, cross-layer conflicts MUST be escalated to human authority.
This protocol's canonical location is the Framework Stack. This instance applies to the Frontend Product Layer pending framework-level formalization.
This protocol determines when Exception Governance must be invoked. Exception Governance defines how to process that invocation. Both protocols must be applied together.
